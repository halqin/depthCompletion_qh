...
% the terms of the BSD license (see the COPYING file).
  ...
  index=find(strcmp(convOpts, 'trainable'));
  if isempty(index) trainable_flag=1, else trainable_flag=convOpts{index+1}, end
  ...
  if ~isempty(opts.size)
    ...
    if isequal(opts.weightScale, 'allone')
    posArgs{2} = Param('value', ones(opts.size, 'single'), ...
  ...
  if ~trainable_flag
      convOpts{index} = [];
      convOpts{index+1} = [];
      convOpts = convOpts(~cellfun('isempty', convOpts));
  end
