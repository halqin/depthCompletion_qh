...
grandtTh = 5+(15-5)* rand(hy_len,1); %GradientThreshold
numTh = 3+(15-3)* rand(hy_len,1); %NumberOfIterations
conn = 1+(10-1)* rand(hy_len,1);  %Connectivity
Connectivity = ['']
...
%          imdb_new.images.data(:,:,4,image_index) = imbilatfilt(imdb.images.data(:,:,4,image_index),'DegreeOfSmoothing', smooth, 'SpatialSigma', sigma);
         imdb_new.images.data(:,:,4,image_index) = imdiffusefilt(imdb.images.data(:,:,4,image_index),'DegreeOfSmoothing', smooth, 'SpatialSigma', sigma);         
